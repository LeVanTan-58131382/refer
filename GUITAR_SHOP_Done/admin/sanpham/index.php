...
switch ($action) {
    ...
    case 'xem_sanpham':
        ...
        include('sanpham_xemchitiet.php');
        break;
    case 'xoa_sanpham':
    ...
    case 'show_add_edit_form':
        ...
        $product = get_product($product_id);
        $categories = get_categories();
        include('sanpham_them_capnhat.php');
        break;
    case 'them_sanpham':
        ...
        // Validate inputs
        if (
            empty($code) || empty($name) || empty($description) ||
            $price === false || $discount_percent === false
        ) {
            $error_message = 'Thông tin sản phẩm không chính xác.
                      Kiểm tra tất cả các trường và thử lại.';
    ...
    case 'capnhat_sanpham':
        ...
        if (
            ...
            $error_message = 'Thông tin sản phẩm chưa chính xác.
                      Kiểm tra lại tất cả các trường và thử lại.';
            include('../../errors/error.php');
        } else {
            $categories = get_categories();
            update_product(
